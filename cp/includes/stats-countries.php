...
?>
    ...
    <div class="centered-header">
      Country Stats: <?php echo $_REQUEST['log']; ?> Last 24 Hours
    </div>

    <div class="block-center fw-bold ta-center" style="width: 800px; font-size: 110%; margin-bottom: 10px;">
      <a href="index.php?r=_xStatsCountriesShow" class="option<?php if( $_REQUEST['log'] == 'In' ) echo ' option-selected'; ?>" style="width: 32%;">In</a>
      <a href="index.php?r=_xStatsCountriesShow&log=Out" class="option<?php if( $_REQUEST['log'] == 'Out' ) echo ' option-selected'; ?>" style="width: 32%;">Out</a>
      <a href="index.php?r=_xStatsCountriesShow&log=Clicks" class="option<?php if( $_REQUEST['log'] == 'Clicks' ) echo ' option-selected'; ?>" style="width: 32%;">Clicks</a>
    </div>
    ...
    <table class="item-table no-wrap fsize-9pt" border="0" cellspacing="0" cellpadding="4" align="center" style="width: 850px; min-width: 850px;">
      ...
      <tbody>
        <?php
        list($total, $countries) = get_overall_countries(strtolower($_REQUEST['log']));
        $counter = 1;
        $color = '#ececec';
        foreach( $countries as $ctry => $amount ):
